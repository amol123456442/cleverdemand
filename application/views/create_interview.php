...
<body>
    <div class="container my-5">
        <div class="card">
            <div class="card-header">
                <h4 class="text-center"><?= isset($interview) ? "Edit Interview" : "Create New Interview" ?></h4>
            </div>
            <div class="card-body">
                <div class="mb-3">
                ...
                <?php
                $formAction = isset($interview) ? base_url('createinterviewcontroller/update/' . $interview['id']) : base_url('createinterviewcontroller/store');
                ?>
                <form action="<?= $formAction ?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control" name="title" id="title"
                            value="<?= isset($interview) ? htmlspecialchars($interview['title']) : '' ?>" required onkeyup="generateSlug()" placeholder="Enter interview title">
                    </div>
                    <div class="mb-4">
                        <label for="slug" class="form-label required-field">Slug</label>
                        <div class="input-group">
                            <input type="text" class="form-control" name="slug" id="slug"
                                value="<?= isset($interview) ? htmlspecialchars($interview['slug']) : '' ?>" required readonly>
                            <button class="btn btn-outline-secondary" type="button" onclick="generateSlug()">
                                <i class="fas fa-sync-alt"></i> Regenerate
                            </button>
                        </div>
                        <small class="text-muted">This will be used in the URL for the interview</small>
                    </div>
                    <div class="mb-4">
                        <label class="form-label required-field">Category</label>
                        <div class="category-select-wrapper">
                            <div class="main-category-select" onclick="toggleMainCategoryDropdown()">
                                <span id="mainCategoryText"><?= isset($interview) && !empty($interview['main_category']) ? htmlspecialchars($interview['main_category']) : 'Select a primary category' ?></span>
                            ...
                            <input type="hidden" name="main_category" id="selectedMainCategory"
                                value="<?= isset($interview) ? htmlspecialchars($interview['main_category']) : '' ?>" required>
                            <input type="hidden" name="category" id="selectedCategory"
                                value="<?= isset($interview) ? htmlspecialchars($interview['category']) : '' ?>" required>
                            <div id="categoryDisplay" class="mt-2">
                                <?php if (isset($interview) && !empty($interview['category'])): ?>
                                    <?php $categories = explode(',', $interview['category']); foreach ($categories as $cat): ?>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select" name="provided" id="provided" required onchange="updateLogoPreview()">
                            ...
                            <option value="Business Wire" data-logo="<?= base_url('assets/logos/business_wire.png') ?>" <?= isset($interview) && $interview['provided'] == "Business Wire" ? "selected" : "" ?>>Business Wire</option>
                            <option value="PR Newswire" data-logo="<?= base_url('assets/logos/pr_newswire.png') ?>" <?= isset($interview) && $interview['provided'] == "PR Newswire" ? "selected" : "" ?>>PR Newswire</option>
                            <option value="PRWeb" data-logo="<?= base_url('assets/logos/PRWeb_Logo.png') ?>" <?= isset($interview) && $interview['provided'] == "PRWeb" ? "selected" : "" ?>>PRWeb</option>
                            <option value="GlobeNewswire" data-logo="<?= base_url('assets/logos/globenewswire.png') ?>" <?= isset($interview) && $interview['provided'] == "GlobeNewswire" ? "selected" : "" ?>>GlobeNewswire</option>
                        </select>
                        <div id="logoPreview" class="mt-2 text-center">
                            <?php if (isset($interview) && !empty($interview['provider_logo'])): ?>
                                <img src="<?= base_url($interview['provider_logo']) ?>" class="logo-preview" alt="Provider Logo">
                            <?php else: ?>
                    ...
                    <div class="mb-4">
                        ...
                        <?php if (isset($interview) && !empty($interview['image'])): ?>
                            <div class="text-center mt-3">
                                <img src="<?= base_url($interview['image']) ?>?t=<?= time() ?>" id="imagePreview" class="image-preview">
                                <div class="mt-2">
                    ...
                    <h5 class="section-title"><i class="fas fa-user-tie me-2"></i>Interviewee Details</h5>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="interviewee_name" class="form-label required-field">Name</label>
                            <input type="text" class="form-control" name="interviewee_name" id="interviewee_name"
                                value="<?= isset($interview) ? htmlspecialchars($interview['interviewee_name']) : '' ?>" required placeholder="Full name">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="interviewee_designation" class="form-label required-field">Designation</label>
                            <input type="text" class="form-control" name="interviewee_designation" id="interviewee_designation"
                                value="<?= isset($interview) ? htmlspecialchars($interview['interviewee_designation']) : '' ?>" required placeholder="Job title">
                        </div>
                    </div>
                    <div class="mb-4">
                        <label for="intervieweeBio" class="form-label">Bio</label>
                        <textarea class="form-control" name="interviewee_bio" id="intervieweeBio" rows="4" placeholder="Brief professional background"><?= isset($interview) ? htmlspecialchars($interview['interviewee_bio']) : '' ?></textarea>
                    </div>
                    <h5 class="section-title"><i class="fas fa-building me-2"></i>Company Details</h5>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="company_name" class="form-label required-field">Company Name</label>
                            <input type="text" class="form-control" name="company_name" id="company_name"
                                value="<?= isset($interview) ? htmlspecialchars($interview['company_name']) : '' ?>" required placeholder="Company name">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="company_industry" class="form-label required-field">Industry</label>
                            <input type="text" class="form-control" name="company_industry" id="company_industry"
                                value="<?= isset($interview) ? htmlspecialchars($interview['company_industry']) : '' ?>" required placeholder="Industry sector">
                        </div>
                    </div>
                    <div class="mb-4">
                        <label for="companyDescription" class="form-label">Description</label>
                        <textarea class="form-control" name="company_description" id="companyDescription" rows="4" placeholder="Brief company overview"><?= isset($interview) ? htmlspecialchars($interview['company_description']) : '' ?></textarea>
                    </div>
                    <h5 class="section-title"><i class="fas fa-edit me-2"></i>Interview Content</h5>
                    <div class="mb-4">
                        <label for="interviewBody" class="form-label required-field">Content</label>
                        <textarea name="content" id="interviewBody" class="form-control"><?= isset($interview) ? htmlspecialchars($interview['content']) : '' ?></textarea>
                    </div>
                    <div class="d-grid mt-4">
                        <button type="submit" class="btn btn-primary btn-lg">
                            <i class="fas fa-save me-2"></i><?= isset($interview) ? "Update Interview" : "Publish Interview" ?>
                        </button>
                    </div>
                </form>
    ...
    <script>
        tinymce.init({
            selector: '#interviewBody, #intervieweeBio, #companyDescription',
