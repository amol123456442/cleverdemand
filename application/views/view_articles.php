...
<head>
    ...
    <title><?= htmlspecialchars($press_release['title'] ?? 'Press Release') ?> - Article</title>
...
<body>
    ...
    <nav class="container mt-3" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="<?= base_url('news') ?>">Article</a></li>
            <li class="breadcrumb-item"><a href="<?= base_url('news?category=' . urlencode($articles['main_category'] ?? 'MarTech')) ?>">MarTech</a></li>
            <li class="breadcrumb-item"><a href="<?= base_url('news?category=' . urlencode($articles['main_category'] ?? 'MarTech') . '&subcategory=Content Management') ?>">Content Management</a></li>
            <li class="breadcrumb-item active" aria-current="page"><?= htmlspecialchars($articles['title'] ?? 'News Article') ?></li>
        </ol>
    </nav>
    ...
    <div class="container py-4">
        <div class="row d-flex bgColor justify-content-between p-3">
            ...
            <div class="col-md-5 d-flex align-items-center">
                <div>
                    <h2 class="fw-bold"><?= htmlspecialchars($articles['title'] ?? 'Untitled') ?></h2>
                    <p class="text-danger"><?= htmlspecialchars($articles['main_category'] ?? 'Uncategorized') ?></p>
                    <small>
                        By <?= htmlspecialchars($articles['provided'] ?? 'Unknown Provider') ?> |
                        Date: <?= date('d M Y', strtotime($articles['created_at'] ?? date('Y-m-d'))) ?> |
                        <?= htmlspecialchars($articles['read_time'] ?? '5 Mins Read') ?>
                    </small>
                </div>
            </div>
        ...
        <div class="row mt-5">
            ...
            <div class="col-md-3">
                <div class="sticky-sidebar">
                    ...
                    <ul>
                        <?php
                        $subcategories = array_map('trim', explode(',', $articles['category'] ?? ''));
                    ...
                    <div class="mb-4">
                        ...
                        <div style="display: flex; gap: 15px;">
                            <a href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode(base_url('news/view/' . $articles['slug'])) ?>"
                            ...
                            <a href="https://twitter.com/intent/tweet?url=<?= urlencode(base_url('news/view/' . $articles['slug'])) ?>&text=<?= urlencode($articles['title']) ?>"
                            ...
                            <a href="https://www.linkedin.com/shareArticle?mini=true&url=<?= urlencode(base_url('news/view/' . $articles['slug'])) ?>&title=<?= urlencode($articles['title']) ?>"
            ...
            <div class="col-md-6">
                <p><?= $articles['content'] ?? 'No content available.' ?></p>
