...
class CreatePressReleaseController extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->database();
        $this->load->model('CreatePressReleaseModel');
    ...
    public function index()
    {
        ...
        $this->load->view('createpressrelease', $data);
    }
    ...
    public function createpressrelease($id = null)
    {
        ...
        try {
            $data['posts'] = $this->CreatePressReleaseModel->getUserPosts($user_id);
            $data['username'] = $this->session->userdata('username');
            $data['role'] = $this->session->userdata('role');

            if ($id) {
                $data['post'] = $this->CreatePressReleaseModel->getPostById($id);
                if (!$data['post'] || $data['post']['user_id'] != $user_id) {
                    $this->session->set_flashdata('error', 'Press release not found or unauthorized.');
                    redirect('createpressrelease');
                }
            } else {
                $data['post'] = null;
            }
    ...
    public function store()
    {
        ...
        if ($this->CreatePressReleaseModel->insert($data)) {
            $this->session->set_flashdata('success', 'Press release created successfully!');
        } else {
            $this->session->set_flashdata('error', 'Failed to create press release. Please try again.');
        }
    ...
    public function allpressreleases()
    {
        ...
        $data['username'] = $this->session->userdata('username');
        $data['role'] = $this->session->userdata('role');
        $this->load->view('createpressrelease/allpressreleases', $data);
    }
    ...
    {
        $main_category = $this->input->post('main_category');
        $validSubcategories = $this->CreatePressReleaseModel->getValidSubcategories($main_category);
